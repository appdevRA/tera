		...
		if request.method == 'POST':
			if 'buttonlogin' in request.POST:
				...
				if user is not None:
					...
					messages.info(request, 'You entered incorrect username or password')
...
class TeraAccountSettingsView(View):
	def get(self, request):
	...
	def post(self, request):
		if request.method == 'POST':
			...
			if form.is_valid():
				...
				messages.info(request, 'Password was unsuccessfully updated.')
		else:
